...
//data
import { loadGradersmanagementListData, popDeleteDialog, InteprateGradersmanagementEvent  } from '../dataControl/GradersmanagementRequestHandler';
...
export default function GradersmanagementList({ dataIn = {}, dataOut = {} }) {
  
  //incoming data in from parent
  const {
    customQueryStr = "",
    customProfilePath="../graders/profile",
  ...
  useEffect(() => {
    ...
    loadGradersmanagementListData(customQueryStr, stateItemSetters);
  ...
  return (
    ...
    <div className={`col-md-12 bg-white p-0 m-0  ${showDataControlSections && ("main_list_container")}  `} style={{marginTop: "0px", paddingBottom: "0px"}}>
      <form method="post" onSubmit={()=>{mosyFilterUrl({tableName:"graders", keyword:stateItem.gradersmanagementQuerySearchStr})}} encType="multipart/form-data">
      
      {showDataControlSections && (<div className="row justify-content-end col-md-12 text-right pt-3 pb-3 data_list_section ml-0 mr-0 mb-3 border-bottom pr-0 pl-0" id="">
        <div className="col-md-6 p-0 text-left pt-3 hive_list_title">
          <h6 className="text-muted"><b> Graders Management </b></h6>
        </div>
        <div className="col-md-6 p-0 text-right hive_list_search_tray">
          <input type="text" id="txt_graders" name="txt_graders" className="custom-search-input form-control" placeholder="Search in Graders Management "
          onChange={(e) => stateItemSetters.setGradersmanagementQuerySearchStr(e.target.value)}
          />
        ...
        <div className="row justify-content-end m-0 p-0 col-md-12 hive_list_action_btn_tray" id="">
          ...
          <div className="col-md-12 p-0 hive_list_nav_right_ribbon" id="">
            ...
            <AddNewButton src="GradersmanagementList" link={customProfilePath} label="New Grader" icon="user" />
          </div>
        </div>
      </div> )}
      ...
      <div className="table-responsive  data-tables bg-white bottom_tbl_handler">
        ...
        <div className="text-left m-0 p-0 col-md-12">
          ...
          onClick={() => {mosyPrintToPdf({elemId : "graders_print_card", defaultTitle:"Graders Management"})}}
          >
          <i className="fa fa-print "></i> Print List
        </div>
        <div className="cpointer p-2 ml-2 badge rounded border border_set badge-whte mb-3 tbl_print_to_excel_btn"
        
        onClick={() => exportTableToExcel("graders_data_table", "Graders Management.xlsx")}
        >
    ...
    <div className="col-md-12 m-0 p-0" id="graders_print_card">
      <table className="table table-hover  text-left printTarget" id="graders_data_table">
        ...
        <tbody>
          {stateItem.gradersmanagementLoading ? (
            <tr>
              <th scope="col">#</th>
              <td colSpan="7" className="text-muted">
                <h5 className="col-md-12 text-center p-3 mb-5 text-muted"><i className="fa fa-spinner fa-spin"></i> Loading Graders Management ...</h5>
              </td>
            </tr>
          ) : stateItem.gradersmanagementListData?.length > 0 ? (
            stateItem.gradersmanagementListData.map((listgraders_result, index) => (
              <Fragment key={`_row_${listgraders_result.primkey}`}>
                <tr key={listgraders_result.primkey}>
                  ...
                  <td scope="col"><span title={listgraders_result.grader_id}>{magicTrimText(listgraders_result.grader_id, 70)}</span></td>
    ...
    <MosyPaginationUi
    src="GradersmanagementList"
    tblName="graders"
    totalPages={stateItem.gradersmanagementListPageCount}
